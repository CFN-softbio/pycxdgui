...
conda install -c softmatter pims
"""
import re
import os
import numpy as np
...
class EigerImages(FramesSequence):
    ...
    def __init__(self, master_filepath,imgthresh=None):
        ...
        if len(lengths) == 0:
            raise KeyError("Lengths array is zero, maybe there's no data here?")
        ...
        if((self.version != b'1.3.0')*(self.version != b'1.5.0')*(self.version != b'1.6.3')):
